      ...
      <svg className="chart-svg" viewBox={`0 0 ${w} ${h}`} role="img" aria-label="Grafico storico prezzi">
        <defs>
          <linearGradient id="chartFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="rgba(158,240,26,0.35)" />
            <stop offset="100%" stopColor="rgba(158,240,26,0)" />
          </linearGradient>
        </defs>
        <line className="chart-avg" x1={padX} x2={w - padX} y1={avgY} y2={avgY} />
        <path className="chart-area" d={area} />
        <path className="chart-line" d={line} />
        <text x={padX} y={14} fill="#5c726c" fontSize="11">
          Media {currency}
          {avgPrice.toFixed(2)}
        </text>
        <text x={padX} y={h - 4} fill="#5c726c" fontSize="11">
          {first.date}
        </text>
        <text x={w - padX} y={h - 4} fill="#5c726c" fontSize="11" textAnchor="end">
          {last.date}
        </text>
      </svg>
    </div>
